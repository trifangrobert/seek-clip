...
const registerUser = async (req: any, res: any) => {
  ...
  try {
    ...
    res.status(200).json({email: user["email"], username: user["username"], firstName: user["firstName"], lastName: user["lastName"], token, _id: user["_id"], profilePicture: user["profilePicture"], premium: user["premium"]});
  } catch (error: any) {
    console.log(error);
    if (error.code === 11000) {
